...
const INCOME_TYPES = [
  { value: "Advertisement – Banner",       label: "Advertisement – Banner" },
  { value: "Advertisement – Bill",         label: "Advertisement – Bill" },
  { value: "Advertisement – Flex",         label: "Advertisement – Flex" },
  { value: "Advertisement – Others",       label: "Advertisement – Others (with description)" },
  { value: "Membership",                   label: "Membership" },
  { value: "Donation",                     label: "Donation" },
  { value: "Outside Bill Collection",      label: "Outside Bill Collection" },
  { value: "Miscellaneous / Others",       label: "Miscellaneous / Others (with description)" },
];

const EXPENSE_TYPES = [
  { value: "Light & Sound",        label: "Light & Sound" },
  { value: "Idol",                 label: "Idol" },
  { value: "Pandal and Decorators",label: "Pandal and Decorators" },
  { value: "Puja Specific",        label: "Puja Specific" },
  { value: "Ashtami Bhog",         label: "Ashtami Bhog" },
  { value: "Purohit",              label: "Purohit" },
  { value: "Helper",               label: "Helper" },
  { value: "Dhaaki Foods",         label: "Dhaaki Foods" },
  { value: "Dhaaki",               label: "Dhaaki" },
  { value: "Coolie",               label: "Coolie" },
  { value: "Transports & Fare",    label: "Transports & Fare" },
...
function Toast({ msg }) {
  if (!msg) return null;
  return <div style={{ position:"fixed", bottom:96, left:"50%", transform:"translateX(-50%)", background:C.darkBg, color:"#fff", padding:"10px 22px", borderRadius:30, fontSize:13, zIndex:300, boxShadow:"0 4px 16px rgba(0,0,0,0.3)", whiteSpace:"nowrap" }}>{msg}</div>;
}
...
function HomePage({ isAdmin, setPage, toast }) {
  ...
  async function save() {
    ...
    try {
      ...
      setClub(form); setEditing(false); toast("✅ Club details saved!");
...
function DurgaPujaPage({ isAdmin, onBack, toast }) {
  ...
  const blankInc = { type:INCOME_TYPES[0].value, amount:"", date:"", depositedTo:"", description:"" };
  const blankExp = { type:EXPENSE_TYPES[0].value, amount:"", date:"", description:"" };
  ...
  async function addIncome() {
    if (!incForm.amount||!incForm.date){ toast("⚠️ Amount & Date are required"); return; }
    setSaving(true);
    try {
      const row = { id:uid(), eventId:"durga2026", ...incForm, amount:parseFloat(incForm.amount) };
      await sheetAppend(TABS.INCOMES, row);
      setIncs(prev=>[...prev,row]);
      setShowInc(false); setIncForm(blankInc);
      toast("✅ Income entry saved!");
    } catch(e){ toast("❌ Failed: "+e.message); }
    setSaving(false);
  }

  async function addExpense() {
    if (!expForm.amount||!expForm.date){ toast("⚠️ Amount & Date are required"); return; }
    setSaving(true);
    try {
      const row = { id:uid(), eventId:"durga2026", ...expForm, amount:parseFloat(expForm.amount) };
      await sheetAppend(TABS.EXPENSES, row);
      setExps(prev=>[...prev,row]);
      setShowExp(false); setExpForm(blankExp);
    ...
    <div style={S.page}>
      ...
      {tab==="finance" && (
        <>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8, marginBottom:12 }}>
            {[{ l:"Total Income", v:totalInc, c:C.success },
              { l:"Total Expense",v:totalExp, c:C.danger  },
              { l:"Cash in Hand", v:cash,     c:cash>=0?C.success:C.danger }].map(x=>(
          ...
          <div style={S.card}>
            ...
            {Object.keys(incByType).length>0 && (
              <div style={{ marginBottom:10 }}>
                <div style={{ fontSize:12, fontWeight:700, color:C.muted, marginBottom:6 }}>Category Summary</div>
              ...
              : incs.map(inc=>(
                <div key={inc.id} style={{ background:"#F0FFF4", borderRadius:8, padding:10, marginBottom:8, border:`1px solid ${C.success}33` }}>
                  ...
                  <div style={{ fontSize:11, color:C.muted }}>
                    📅 {inc.date}
                    {inc.depositedTo && <span> | 👤 Deposited to: <strong>{inc.depositedTo}</strong></span>}
                  </div>
          ...
          <div style={S.card}>
            ...
            {Object.keys(expByType).length>0 && (
              <div style={{ marginBottom:10 }}>
                ...
                <div style={{ ...S.btwn, padding:"8px 0", marginTop:4 }}>
                  <span style={{ fontSize:13, fontWeight:700 }}>Total Expense</span>
      ...
      {tab==="photos" && (
        <div style={{ ...S.card, ...S.stub }}>
          ...
          <div style={{ fontWeight:700, color:C.deepRed, marginTop:10 }}>Photos Coming Soon</div>
          <div style={{ fontSize:13, marginTop:6, color:C.muted }}>Year-wise event photos will appear here.</div>
        </div>
      )}

      {/* ADD INCOME MODAL */}
      <Modal open={showInc} onClose={()=>setShowInc(false)} title="📥 Add Income Entry">
        <label style={S.label}>Income Type / আয়ের ধরন *</label>
        <select style={S.select} value={incForm.type} onChange={e=>setIncForm({...incForm,type:e.target.value})}>
          {INCOME_TYPES.map(t=><option key={t.value} value={t.value}>{t.label}</option>)}
        </select>

        <label style={S.label}>Amount / পরিমাণ (₹) *</label>
        ...
        <label style={S.label}>Date / তারিখ *</label>
        <input style={S.input} type="date" value={incForm.date} onChange={e=>setIncForm({...incForm,date:e.target.value})} />

        <label style={S.label}>Deposited To (Person Name) / যাকে দেওয়া হয়েছে</label>
        <input style={S.input} value={incForm.depositedTo} onChange={e=>setIncForm({...incForm,depositedTo:e.target.value})} placeholder="Name of person who received this amount..." />

        <label style={S.label}>Description / Comments — বিবরণ</label>
        <textarea style={S.textarea} value={incForm.description} onChange={e=>setIncForm({...incForm,description:e.target.value})} placeholder="Source name, advertiser name, donor name, or any other details..." />
      ...
      <Modal open={showExp} onClose={()=>setShowExp(false)} title="📤 Add Expense Entry">
        <label style={S.label}>Expense Type / ব্যয়ের ধরন *</label>
        ...
        <label style={S.label}>Description / Comments — বিবরণ</label>
        <textarea style={S.textarea} value={expForm.description} onChange={e=>setExpForm({...expForm,description:e.target.value})} placeholder="What was this expense for, vendor name, receipt details..." />
    ...
    <div style={S.page}>
      ...
      {events.map(ev=>(
        <div key={ev.id} style={{ ...S.card, cursor:ev.id==="durga2026"?"pointer":"default" }}
          ...
          {ev.id==="durga2026" && <div style={{ fontSize:12, color:C.saffron, marginTop:4, fontWeight:600 }}>Tap to view full details & finance →</div>}
        </div>
      ))}
...
function AdminPortal({ admin, onLogout, toast, setPage, setSubPage }) {
  ...
  async function delMember(id) {
    ...
    try { await sheetDeleteRow(TABS.MEMBERS,id); setMembers(p=>p.filter(m=>m.id!==id)); toast("Member deleted."); }
    catch { toast("❌ Delete failed."); }
  }
    ...
    <div style={S.page}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:12 }}>
        <button onClick={()=>{ setPage("events"); setSubPage("durga2026"); }}
          ...
          <div style={{ fontSize:24, marginBottom:4 }}>₹</div>Add Income / Expense<br/><span style={{ fontSize:10, fontWeight:400, color:C.muted }}>Durga Puja 2026</span>
        </button>
      ...
      {tab==="guide" && (
        <div style={S.card}>
          ...
          <div style={{ fontSize:13, color:"#444", lineHeight:2.1 }}>
            <div>💰 <strong>Add Income/Expense</strong><br/>&nbsp;&nbsp;→ Events → Durga Puja 2026 → Finance tab<br/>&nbsp;&nbsp;→ Or use the quick button above</div>
            <div style={S.divider} />
            <div>👥 <strong>Add Members</strong><br/>&nbsp;&nbsp;→ Members tab above</div>
            <div style={S.divider} />
            <div>🏠 <strong>Edit Club Details</strong><br/>&nbsp;&nbsp;→ Home Page → ✏️ Edit button on banner</div>
            <div style={S.divider} />
            <div>📊 <strong>View all data in Google Sheet</strong><br/>&nbsp;&nbsp;→ <a href="https://docs.google.com/spreadsheets/d/1VAM7ajyEg7J99zbBScthfFkXyncdCZ29ceLSF6OZFqU" target="_blank" rel="noreferrer" style={{ color:C.saffron }}>Open OUS-Database ↗</a></div>
            <div style={S.divider} />
            <div>🔑 <strong>Change passwords</strong><br/>&nbsp;&nbsp;→ Edit directly in the Admins tab of the Google Sheet</div>
          </div>
        </div>
      )}
      ...
      <Modal open={showMember} onClose={()=>setShowMember(false)} title="👤 Add New Member">
        {[["name","Full Name *"],["nameBn","Bengali Name / বাংলা নাম"],["phone","Phone Number"],["email","Email"],["address","Address / ঠিকানা"],["joinDate","Join Date"]].map(([f,l])=>(
          <div key={f}>
            <label style={S.label}>{l}</label>
...
export default function App() {
  ...
  useEffect(()=>{
    initSheets().then(()=>setReady(true)).catch(()=>{ toast("⚠️ Google Sheets connection issue."); setReady(true); });
  },[]);
